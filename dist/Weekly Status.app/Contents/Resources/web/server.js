...
const ROOT_DIR = process.env.WEEKLY_STATUS_STATIC_ROOT
  ? path.resolve(process.env.WEEKLY_STATUS_STATIC_ROOT)
  : __dirname;
const DATA_DIR = process.env.WEEKLY_STATUS_DATA_DIR
  ? path.resolve(process.env.WEEKLY_STATUS_DATA_DIR)
...
function normalizeState(candidate) {
  ...
  const safeColumns = candidate.taskColumns && typeof candidate.taskColumns === 'object'
    ? candidate.taskColumns
    : DEFAULT_STATE.taskColumns;

  return {
    ...candidate,
    tasks: Array.isArray(candidate.tasks) ? candidate.tasks : [],
    taskSortMode: typeof candidate.taskSortMode === 'string'
      ? candidate.taskSortMode
      : DEFAULT_STATE.taskSortMode,
    taskColumns: {
      name: Number.isFinite(safeColumns.name) ? safeColumns.name : DEFAULT_STATE.taskColumns.name,
